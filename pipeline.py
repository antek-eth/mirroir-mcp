...
.config-card.starred{border-color:#d97706;box-shadow:0 0 0 1px rgba(217,119,6,.3),0 8px 24px rgba(0,0,0,.3)}
.star-btn{background:none;border:none;cursor:pointer;font-size:16px;color:var(--fg3);padding:2px 4px;line-height:1;border-radius:4px;transition:all .15s;flex-shrink:0}
.star-btn:hover{color:#d97706}
.star-btn.active{color:#f59e0b}
.starred-label{font-family:var(--font-mono);font-size:10px;text-transform:uppercase;letter-spacing:1px;color:#d97706;padding:4px 0 8px;grid-column:1/-1;display:flex;align-items:center;gap:8px}
.starred-label::after{content:'';flex:1;height:1px;background:rgba(217,119,6,.3)}
...
<div class="container">
  ...
  <div class="filters">
    ...
    <div class="filter-group">
      <button class="chip-btn" id="starOnlyBtn" onclick="starOnly=!starOnly;this.classList.toggle('active');applyFilters()">&#9733; Starred</button>
    </div>
    <div class="filter-group">
...
let starOnly = false;
function getStarred() { try { return JSON.parse(localStorage.getItem('starred') || '[]'); } catch { return []; } }
function saveStarred(keys) { localStorage.setItem('starred', JSON.stringify(keys)); }
...
function applyFilters() {
  ...
  filtered = configs.map(c => {
    ...
    if (starOnly && !isStarred(c.key)) return false;
    return true;
  });
...
function render() {
  ...
  const sortedFiltered = [...filtered].sort((a, b) => {
    const as = starred.includes(a.key) ? 0 : 1;
    const bs = starred.includes(b.key) ? 0 : 1;
    return as - bs;
  });
  let firstUnstarred = true;
  grid.innerHTML = sortedFiltered.map((c, ci) => {
    const cc = chipClass(c.cpu);
    const label = (c.screen ? c.screen + '" ' : '') + c.cpu;
    const llmVal = c.llm_q4;
    const isStar = starred.includes(c.key);
    const sectionLabel = (!isStar && firstUnstarred && starred.some(k => sortedFiltered.find(x => x.key === k))) ? (firstUnstarred = false, '<div class="starred-label">&#9733; Starred above &nbsp;&nbsp; All configs</div>') : (isStar ? '' : (firstUnstarred = false, ''));
    return `${sectionLabel}
    <div class="config-card${isStar ? ' starred' : ''}">
      <div class="config-header">
        ...
        <button class="star-btn${isStar ? ' active' : ''}" onclick="event.stopPropagation();toggleStar('${c.key}')" title="${isStar ? 'Unstar' : 'Star this config'}">\u2605</button>
...
(function(){
  ...
  function renderSummary(s){
    ...
    actions.forEach(function(a){
      var msg = (a && typeof a === 'object') ? a.msg : a;
      issues += '<div class="action">action required: ' + esc(msg) + '</div>';
    });
